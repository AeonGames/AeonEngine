...
namespace AeonGames
{
    ...
    void OpenGLWindow::BeginPointShadowPass ( uint32_t aCaster )
    {
        ...
        // face matrices and the light position/radius, and routes each face to
        // its cube-array layer. base_layer is absolute here (caster*6) because
        // the whole cube-map array is attached as a layered target.
        ...
        // The layered attachment covers every caster's layers, so clear the
        // whole array once, before the first caster, to avoid wiping a caster
