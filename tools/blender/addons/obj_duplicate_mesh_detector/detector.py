...
def apply_transform(point_cloud, rotation, translation,
                    scale: "float | tuple[float, ...]" = 1.0):
    """Apply a similarity transform (uniform or per-axis scale + rotation +
    translation) to a point cloud. ``scale`` may be a float or a 3-tuple."""
...
def compare_uv_data(uv_data1, uv_data2, tolerance):
    ...
    for uvs1 in uv_data1.values():
        for uvs2 in uv_data2.values():
...
def compare_point_clouds_translation_only(cloud1, cloud2, centroid1, centroid2,
                                          tolerance, match_threshold=100.0):
    """
    Order-invariant fallback: translate cloud1 so its centroid lands on cloud2's
    centroid, then do a spatial nearest-neighbour match. This catches duplicate
    meshes that are translated copies of each other but whose vertex order has
    been shuffled (e.g. by an export/import round-trip), which the
    correspondence-by-index Umeyama path cannot detect.

    Rotation is NOT estimated here. For rotated duplicates the Umeyama path is
    ...
    if not cloud1:
        return True, 100.0
    sx = centroid2[0] - centroid1[0]
    sy = centroid2[1] - centroid1[1]
    sz = centroid2[2] - centroid1[2]
    shifted = [(x + sx, y + sy, z + sz) for (x, y, z) in cloud1]
    return compare_point_clouds_internal(
        shifted, cloud2, tolerance, match_threshold, verbose=False)
...
def compare_pair(data1, data2, tolerance, compare_uvs, match_threshold=100.0, allow_scale=False, allow_non_uniform_scale=False):
    ...
    # ---- EARLY OUT: face / edge count (cheapest topology check) ----
    if data1['face_count'] != data2['face_count']:
        return (data1['name'], data2['name'], False, 0.0, 1.0)
    if data1['edge_count'] != data2['edge_count']:
        return (data1['name'], data2['name'], False, 0.0, 1.0)

    # ---- EARLY OUT: bounding-box extents (rigid mode only) ----
    ...
    # ---- FAST ACCEPT: identical local-space hash (rigid mode only) ----
    # The hash is computed from quantized, sorted vertex coordinates, so it is
    # order-invariant. A hash match means the two clouds are identical up to
    # quantization — verify cheaply and accept without running Umeyama.
    if (not allow_scale and not allow_non_uniform_scale
            and data1['hash'] == data2['hash']):
        is_duplicate, match_percentage = compare_point_clouds_internal(
            data1['point_cloud'], data2['point_cloud'],
            tolerance, match_threshold, verbose=False)
        if is_duplicate:
            print(f"  [FAST-ACCEPT hash] '{data1['name']}' <-> '{data2['name']}' "
                  f"({match_percentage:.2f}%)")
            if compare_uvs:
                is_duplicate = compare_uv_data(
                    data1['uv_data'], data2['uv_data'], tolerance)
            return (data1['name'], data2['name'], is_duplicate,
                    match_percentage, 1.0)

    # ---- Full point-cloud comparison (Umeyama, index-correspondence) ----
    ...
    # ---- FALLBACK: order-invariant translation match (rigid mode only) ----
    # Umeyama relies on vertex index correspondence. If the two meshes have the
    # same geometry but different vertex orderings (common after
    # export/import), Umeyama estimates a wrong transform and reports a low
    # match. Try a centroid-aligned spatial nearest-neighbour match as a
    # fallback so we still catch translated copies with shuffled vertices.
    if (not is_duplicate
            and not allow_scale and not allow_non_uniform_scale):
        fb_is_dup, fb_pct = compare_point_clouds_translation_only(
            data1['point_cloud'], data2['point_cloud'],
            data1['centroid'], data2['centroid'],
            tolerance, match_threshold)
        if fb_is_dup:
            print(f"  [FALLBACK translation] '{data1['name']}' <-> "
                  f"'{data2['name']}' Umeyama missed, centroid-aligned "
                  f"spatial match: {fb_pct:.2f}%")
            is_duplicate = True
            match_percentage = fb_pct
            estimated_scale = 1.0
...
def find_duplicate_meshes(objects, tolerance, use_world_space, compare_uvs=False, match_threshold=100.0,
    ...
    # Group by full topology fingerprint (vertex/face/edge counts).
    # Two meshes whose vertex, face, or edge counts differ cannot be
    # duplicates, so bucketing by all three drastically reduces the number
    # of pair comparisons compared to bucketing by vertex count alone.
    topology_groups = defaultdict(list)
    for data in mesh_data:
        key = (data['vertex_count'], data['face_count'], data['edge_count'])
        topology_groups[key].append(data)

    print(f"\n=== Topology grouping results: {len(topology_groups)} unique "
          f"(v,f,e) keys ===")
    for key, group in topology_groups.items():
        obj_names = [item['name'] for item in group]
        print(f"verts={key[0]} faces={key[1]} edges={key[2]} "
              f"({len(group)} objects): {', '.join(obj_names)}")

    # Find actual duplicates by comparing all objects sharing a topology key.
    # Build all comparison jobs first, then execute in one global thread pool
    # for better utilization across groups.
    duplicate_groups = []
    scale_logs = []

    print("\n=== Starting detailed comparison with multithreading ===")

    comparison_jobs = []  # (topology_key, data1, data2)
    for key, group in topology_groups.items():
        if len(group) < 2:
            continue

        pair_count = (len(group) * (len(group) - 1)) // 2
        print(f"\nTopology {key}: {pair_count} pairs")
        for i, data1 in enumerate(group):
            for j in range(i + 1, len(group)):
                comparison_jobs.append((key, data1, group[j]))
    ...
    matches_by_topology = defaultdict(list)
    with ThreadPoolExecutor(max_workers=min(len(comparison_jobs), _CPU_COUNT)) as executor:
        futures = {
            ...
            for key, data1, data2 in comparison_jobs
        }
        ...
        for future in as_completed(futures):
            ...
            if is_duplicate:
                matches_by_topology[key].append((name1, name2))
    ...
    # Build connected components per topology bucket.
    for _key, matches in matches_by_topology.items():
        ...
        for obj_name in group[1:]:
            ...
            if dup_mesh.uv_layers:
                ...
                if source_uv:
                    ...
                    matching_uv_name = find_matching_uv_layer(base_mesh, source_uv)
...
def find_matching_uv_layer(base_mesh, source_uv, tolerance=0.0001):
    """
...
def copy_uv_data(source_uv, dest_uv):
    """
    Copy UV data from source to destination UV layer.
    Assumes meshes have the same topology (loop order matches).
